...
static void enqueue_sjn(process *p, queue_node *queue){
    if (queue->next == NULL){
        enqueue(p, queue);
        return;
    } 
    ...
    while(curr->next != NULL){
        ...
        curr = curr->next;
        proc = curr->object;
        if (p->burst_time >= proc->burst_time){
            enqueue(p, prev);
            return;
        }
    }
...
static void enqueue_arrivals(process p[], int n, int time, queue_node *queue){
	for (int j = 0; j < n; j++){
		if (p[j].arrival_time == time){
			enqueue_sjn(&p[j], queue);
		}
	}
}

void sjn(process p[], int n, int maxTime, char result[]){
