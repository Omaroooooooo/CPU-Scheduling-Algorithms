...
static void enqueue_prio_pr(process *p, queue_node *queue){
    if (queue->next == NULL){
        enqueue(p, queue);
        return;
    } 
...
static void enqueue_arrivals(process p[], int n, int time, queue_node *queue){
	for (int j = 0; j < n; j++){
		if (p[j].arrival_time == time){
			enqueue_prio_pr(&p[j], queue);
		}
	}
}

void prio_pr(process p[], int n, int maxTime, char result[]){
	...
	while (time < maxTime){
		...
		if (running != NULL){
			...
			if (proc != NULL && running->priority < proc->priority){
				enqueue_prio_pr(running, queue);
